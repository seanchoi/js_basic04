...
//       let highestRankMovie = movies[j]; //movies[j] -> object in index j
//       let max_location = j;

//       for (let i = j; i < movies.length; i++) {
//           if (movies[i].rank > highestRankMovie.rank) { // indext object and comparing element
//               // Know max AND it's index (location)
//               highestRankMovie = movies[i]
//               max_location = i
//           }
//       }
//       // swap the first and the last
//       movies[max_location] = movies[j] // --> 10
//       movies[j] = highestRankMovie
//   }
...
function sortMoviesByAttr(movies, sortAttr){
  ...
  for (let j = 0; j < movies.length - 1; j++) {
    ...
    let newMax = getMaxMovieObject(movies, j, sortAttr);
    highestRankMovie = newMax.top_list;
    max_location = newMax.max_index;

    // for (let i = j; i < movies.length; i++) {
    //     if (movies[i][sortAttr] > highestRankMovie[sortAttr]) { // indext object and comparing element
    //         // Know max AND it's index (location)
    //         highestRankMovie = movies[i]
    //         max_location = i
    //     }
...
function getMaxMovieObject(movies, start, sortAttr){
  ...
  let max_movie = movies[start];
  let max_location = start

  for (let i = start; i < movies.length; i++) {
      if (movies[i][sortAttr] > max_movie[sortAttr]) {
          max_movie = movies[i]
          max_location = i
      }
  }
  return {top_list: max_movie, max_index: max_location}
}
